...
local function my_theme_fun(mode_color)
  ...
  if color_name then
    ...
    if ok and theme then
      --print("Loaded theme:", theme)    -- Debugging statement to print the theme
      --print("Mode color:", mode_color) -- Debugging statement to print the mode_color
      for section, contents in pairs(theme) do
        if section ~= 'inactive' then
          if contents.a and contents.b then -- Check if subtables a and b exist
            contents.a.bg = theme[mode_color].a and theme[mode_color].a.bg or contents.a.bg
            contents.b.fg = theme[mode_color].b and theme[mode_color].b.fg or contents.b.fg
          else
            --print("Missing subtable in section:", section) -- Debugging output if subtables do not exist
          end
        end
      end
  ...
  local M = {
    normal = {
      a = { bg = colors[mode_color], fg = colors.back1, gui = 'bold' },
      b = { bg = colors.back1, fg = colors[mode_color] },
